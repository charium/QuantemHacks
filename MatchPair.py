...
# -----------------------------def map_user_to_params(user):
    ...
    return theta, phidef prepare_user_circuit(theta, phi):
    # Prepare single-qubit state using RY and RZ
    qc = QuantumCircuit(1)
    qc.ry(theta, 0)
    qc.rz(phi, 0)
    return qcdef compare_users(user1, user2):
    # Build a 2-qubit circuit to compare two users
    theta1, phi1 = map_user_to_params(user1)
    theta2, phi2 = map_user_to_params(user2)    qc = QuantumCircuit(2, 2)
    qc.ry(theta1, 0)
    qc.rz(phi1, 0)
    qc.ry(theta2, 1)
    qc.rz(phi2, 1)
...
scores = list(match_scores.values())plt.figure(figsize=(10, 5))
